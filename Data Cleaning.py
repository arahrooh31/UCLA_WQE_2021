...
outcomes_cleaned.fillna("bfill", inplace = True)
...
medical_history_cleaned.fillna("bfill", inplace = True)
...
print(cleaned_heart_disease_data.isnull().sum())
...
#Splitting the Data
X = pd.read_csv('cleaned data/cleaned_heart_disease_data.csv', usecols = range(1,188), header = 0)
Y = pd.read_csv('cleaned data/cleaned_heart_disease_data.csv', usecols = range(188,198), header = 0)
...
X_train.to_csv(r'cleaned data/split data/X_test.csv', index = False)
X_test.to_csv(r'cleaned data/split data/X_train.csv', index = False)
